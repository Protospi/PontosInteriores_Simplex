...
text-align: justify}
  ...
  _O método de otimização Simplex possui a característica de encontrar a solução exata em um problema de programação linear, porém, em problemas de grande dimensão, o algoritmo apresenta alto custo computacional para atingir a convergência. O método de optimização Pontos Interiores apresenta baixo custo computacional em problemas de grande dimensão, mas apresenta resultados que apenas aproximam a solução ótima. O objetivo deste projeto é ilustrar o funcionamento desses dois algoritmos, compará-los por meio de simulações para identificar suas vantagens e desvantagens e construir um algoritmo de otimização híbrido que unifica os pontos fortes de cada método._
...
* Dimensões superiores a __60__ até __60__ apresentaram custo computacional de __300__ até __500__ iterações.
...
* Dimensões superiores a __60__ apresentaram custo computacional de __10__ até __20__ iterações.
...
* Pontos Interiores apresentou maior mistura de custos, destaque para correlação com número de variáveis. Simplex apresentou regiões mais bem definidas, com poucas observações discrepantes e correlação semelhante com as duas variáveis.
...
* A amplitude de diferenças apresentou variação $[10^{-10}, 10^{-4}]$.

* A magnitude entre a maior e a menor diferença foi da ordem de $10^{-6}$.


### __Conclusão__

* O algoritmo Pontos Interiores apresentou melhor performance na análise do custo computacional. Apesar do algoritmo Simplex apresentar máximos maiores na ordem de até $10^{-4}$, apenas em problemas que demandam precisão científica seu custo computacional elevado poderia ser justificado.
